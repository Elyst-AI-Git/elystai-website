...
function captureUtm(): Record<string, string> {
  ...
  try {
    if (Object.keys(values).length) {
      window.sessionStorage.setItem(UTM_STORAGE_KEY, JSON.stringify(values));
    }
  } catch {
...
function referenceButtonClass(
  variant: "metal" | "outline" | "solid",
  tone: "emerald" | "light" | "green",
) {
  if (variant === "outline") {
    return "border-2! bg-white! text-emerald! transition-colors! hover:bg-emerald/5!";
  }

  if (variant !== "solid") return "";

  if (tone === "light") {
    return "bg-[#eef0ee]! text-[#0a0a0a]! hover:bg-[#e3e6e2]!";
  }
  if (tone === "green") {
    return "bg-green! text-[#06140e]! hover:bg-[#00c973]!";
  }
  return "bg-emerald! text-fg-on-dark! hover:bg-emerald-light!";
}
...
export default function TrackedCta({
  ...
  useEffect(() => {
    const timer = window.setTimeout(
      () => setHref(withUtm(baseHref, captureUtm())),
      0,
    );
  ...
  const mergedClassName = [
    referenceButtonClass(variant, tone),
    className,
  ]
    .filter(Boolean)
    .join(" ");
...
export function TrackedExternalLink({
  ...
  return (
    <a
      href={href}
      onClick={() => recordSchedulerIntent(intent)}
      className={className}
    >
      {children}
    </a>
